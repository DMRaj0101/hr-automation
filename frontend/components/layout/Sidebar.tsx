...
import { NAV_ITEMS, NAV_GROUP_ORDER, isNavItemActive, getNavBadge, type NavItem } from "@/lib/nav-items";
...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      <div className={cn("mb-5 flex items-center", collapsed ? "flex-col gap-4 justify-center px-2" : "justify-between pl-0 pr-1")}>
            ...
            <AnimatePresence mode="wait" initial={false}>
              {!collapsed ? (
                ...
                >
                  ...
                  <span className="whitespace-nowrap text-[9px] font-semibold uppercase tracking-[0.15em] text-vantara-gold/90">
                    {group}
                  </span>
              ...
              {items.map((item: NavItem) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      "group relative flex items-center gap-2.5 text-sm font-medium transition-colors",
                      collapsed ? "h-10 w-10 justify-center rounded-xl" : "rounded-xl px-2 py-2",
                  ...
                  >
                    ...
                    {/* Monogram badge, derived from label — active badge gets a gold glow */}
                    <span
                      className={cn(
                        "relative flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-[10px] font-semibold transition-all",
                        active
      ...
      >
        <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-vantara-gold text-[11px] font-medium text-vantara-navy">
          {(user?.name ?? "HR").slice(0, 2).toUpperCase()}
        </div>
        {!collapsed && (
          <span className="whitespace-nowrap text-sm font-medium text-white">{user?.name ?? "HR Admin"}</span>
        )}
      </div>
    </motion.aside>
  );
