...
| 5 | `app/integrations/openkm_connector.py` | Client Document Repository (Tax), Client Engagement Repository (Audit), Document Management (Law) | **Fully implemented, including real OpenKM user provisioning, and tested end-to-end against a live `openkm/openkm-ce` 6.3.12 container.** Upgraded from Mock to Functional beyond the original PDD Section 3 tables — these were originally spec'd against commercial platforms (NetDocuments/iManage) and would have stayed Mock; OpenKM is open-source so this became feasible. IT Support has no document-management item. **Correction from an earlier pass:** this connector originally assumed Community Edition couldn't create OpenKM user accounts via REST (based on an old forum thread) and only wrote best-effort, unverifiable ACL grants. That was wrong — confirmed by pulling the live instance's own `swagger.json` and round-trip testing `/auth/createUser` → `/auth/assignRole` → `/auth/grantUser` → `/auth/login`, including a negative test (wrong password → 401) to prove the login check was real, not a false positive. `create_workspace()` now creates a real, working OpenKM login for each employee (random per-employee password, meant to be threaded into the welcome email — see the connector's own docstring on the design choice vs. a future Keycloak-SSO alternative), and `grant_verified` is a genuine confirmed boolean (via a `getGrantedUsers` readback) rather than a permanent placeholder. Lesson worth keeping in mind for the other three connectors: prefer a live instance's own swagger/API-explorer output over secondhand docs, and don't trust an endpoint's mere presence in swagger.json either — only an actual round-trip test proves it works. **Second addition:** `mock_hrms/mock_employee_docs/` (and the `documents_files` fixture field) were restored — they'd been deleted in an earlier pruning pass under the mistaken assumption that "document validation is out of scope" also meant "no document files are needed at all." Those are different things: OCR/identity-document validation is genuinely out of scope, but seeding a real document-management workspace with the employee's actual onboarding documents (offer letter, resume, etc.) is a good use of `openkm_connector.py` now that it's real. `SSN_Scanned`/`BankDetails` files are excluded by default (`integrations/hrms_connector.py`'s `SENSITIVE_DOCUMENT_MARKERS`) — flagged as a judgment call for whoever owns document/compliance policy, not silently decided here. |
